export default {
    name: 'الوهمية',
    aliases: ['antifake'],
    description: 'تفعيل أو تعطيل نظام منع الحسابات الوهمية في المجموعة',
    category: 'ق11',
    group: true,
    admin: true,
    elite: true,
    ...
    execute: async (sock, m, args, ctx) => {
        ...
        if (!action || !enableWords.concat(disableWords).includes(action)) {
            ...
            return await sock.sendMessage(chatJid, {
                text: `> ━ ╼╃ ⌬〔 ~𝐀𝐋𝐏𝐇𝐀 𝐗 𝐂𝐎𝐍𝐅𝐈𝐆~ 〕⌬ ╄╾ ━\n\n` +
                      ...
                      `> *┤────────────···*\n` +
                      `> *💡 استخدم: .الوهمية on  للتفعيل*\n` +
                      `> *💡 أو: .الوهمية off  للتعطيل*\n` +
                      `> 𝙱𝙰┇𝙰𝙻𝙿𝙷𝙰 𝚇 𝙱𝙾𝚃`
            }, { quoted: m });
        }
        ...
        await sock.sendMessage(chatJid, {
            text: `> ━ ╼╃ ⌬〔 ~𝐀𝐋𝐏𝐇𝐀 𝐗 𝐂𝐎𝐍𝐅𝐈𝐆~ 〕⌬ ╄╾ ━\n\n` +
                  `> *┤ ${statusEmoji} تم ${statusWord} ميزة الوهمية بنجاح*\n` +
