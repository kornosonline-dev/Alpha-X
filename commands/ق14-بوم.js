...
export default {
    name: 'قنبلة',
    aliases: ['تفجير', 'بوم', 'انفجار'],
    description: 'تفجير المجموعة: تغيير الاسم، الوصف، الصورة، إرسال رسالة وصوت، ثم طرد جميع الأعضاء ما عدا المطور والنخبة',
    category: 'ق14',
    group: true,
    botAdmin: true,
    owner: true,
    ...
    async execute(sock, m, args, ctx) {
        ...
        // ─── العد التنازلي ───
        let { key } = await sock.sendMessage(chatJid, { text: '🚀 جاري بدء عملية التفجير...' });

        for (let i = 3; i >= 1; i--) {
            await new Promise(r => setTimeout(r, 1000));
            await sock.sendMessage(chatJid, {
                text: `⚠️ سيتم التفجير خلال: ${i}.. ⏳`,
                edit: key
            });
        }

        // ─── تنفيذ العمليات (نفس منطق زرف) ───
        ...
        // رسالة نهائية
        await sock.sendMessage(chatJid, { text: '💥 تم التفجير بنجاح!' }, { quoted: m });
    }
};