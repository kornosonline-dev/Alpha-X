...
function sticker6(img, url) {
    return new Promise(async (resolve, reject) => {
        ...
        const tmp = path.join(__dirname, `../temp/${+new Date()}.${type.ext}`);
...
async function sticker5(img, url, packname, author, categories = [""], extra = {}) {
    ...
    try {
        ...
        console.log("wa-sticker-formatter غير متوفر، نستخدم sticker6");
        return null;
    }
}
...
async function sticker(img, url, packname, author, categories = [], extra = {}) {
    ...
    for (const func of funcs) {
        try {
            ...
            throw stiker?.toString() || "فشل التحويل";
        } catch (err) {
            lastError = err;
            continue;
        }
